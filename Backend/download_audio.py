...
def save_metadata(video_id, video_title):
    ...
    try:
        ...
        print(
            f"    ERROR: Failed to save metadata "
            f"for {video_id}: {e}"
        )
        return False
...
def download_one_video(video_id):
    ...
    if not os.path.exists(expected_mp3):
        print(
            f"    ERROR: Download reported OK "
            f"but MP3 not found: {expected_mp3}"
        )
        return False
...
def get_int_input(prompt, default):
    ...
    try:
        ...
        print(
            f"  Invalid number, using default: {default}"
        )
        return default
...
def main():
    ...
    print("\nUsing configured playlist:")
    print(playlist_url)

    print(
        "\nHow many videos do you want "
        "to process in this batch?"
    )

    batch_size = get_int_input(
        "  Batch size (press Enter for default 10): ",
        10
    )
    ...
    start_position = get_int_input(
        "  Start position (press Enter for default 1): ",
        1
    )
    ...
    print(
        f"Playlist: "
        f"{playlist_info.get('title', 'Unknown')}"
    )

    print(
        f"Total videos in playlist: "
        f"{total_in_playlist}"
    )
    ...
    if start_index >= total_in_playlist:
        ...
        fail(
            ...
            f"is beyond the playlist length "
            f"({total_in_playlist})."
        )
    ...
    for offset, entry in enumerate(batch_entries):
        ...
        if not video_id:
            ...
            print(
                f"[{playlist_position}] "
                f"Skipping entry with no video_id."
            )
    ...
    print(
        f"Batch: videos {start_position} to "
    ...
    if failed_videos:
        ...
        print(
            "\nFailed videos "
            "(re-run this same batch to retry):"
        )
    ...
    if next_start <= total_in_playlist:
        ...
        print(
            f"\nNext batch: run again with "
            f"start position {next_start} "
            f"to continue."
        )
